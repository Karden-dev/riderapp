...
import 'package:shared_preferences/shared_preferences.dart'; // L'IMPORT REQUIS
...
class AuthService extends ChangeNotifier {
  ...
  // get SharedPreferences => null; // <-- Ligne fautive supprimée

  // Initialisation : Tente de charger l'utilisateur depuis le stockage local
  Future<void> init() async {
    // Appelle maintenant la VRAIE classe SharedPreferences
  ...
  // *** NOUVELLE MÉTHODE POUR ENREGISTRER LE TOKEN FCM ***
  Future<void> updateFcmToken(String token) async {
    if (!isAuthenticated) return;

    try {
      await _dio.patch(
        '$_apiBaseUrl/deliverymen/fcm-token', // C'est la route backend que nous devrons créer
        data: {'fcm_token': token},
      );
      debugPrint('AuthService: Token FCM mis à jour avec succès.');
    } on DioException catch (e) {
      // Ne pas déconnecter l'utilisateur, juste logguer l'erreur
      debugPrint('AuthService: Échec de la mise à jour du token FCM: ${e.response?.data}');
    }
  }
}