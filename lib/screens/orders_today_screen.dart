...
class _OrdersTodayScreenState extends State<OrdersTodayScreen> {
  ...
  // --- NOUVELLE MODALE DE PLANIFICATION INTUITIVE (Remplace _showDateTimePickerDialog) ---
  Future<DateTime?> _showScheduleModal(BuildContext context, String title, DateTime initialDateTime, bool isRelaunch) async {
    DateTime selectedDateTime = initialDateTime;

    return await showDialog<DateTime>(
      context: context,
      builder: (BuildContext dialogContext) {
        // Utiliser StatefulBuilder pour mettre à jour l'UI de la modale (l'heure/date)
        return StatefulBuilder(
          builder: (context, setModalState) {
            final DateFormat dateFormatter = DateFormat('dd MMM yyyy', 'fr_FR');
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  InkWell(
                    // Pour 'A relancer' (isRelaunch), la date est fixe (aujourd'hui), on ne peut pas la changer
                    onTap: isRelaunch ? null : () async {
                    ...
                    child: Padding(
                      ...
                      child: Text(
                        ...
                        style: Theme.of(context).textTheme.titleMedium?.copyWith(
                          // Grisé si non-modifiable (A relancer), sinon couleur primaire
                          color: isRelaunch ? Colors.grey.shade700 : AppTheme.primaryColor, 
                  ...
                  // --- SÉLECTEUR D'HEURE ---
                  Text('Heure:', style: Theme.of(context).textTheme.bodySmall),
                  InkWell(
                    onTap: () async {
                    ...
                    child: Padding(
                      padding: const EdgeInsets.symmetric(vertical: 12.0),
                      child: Text(
                        timeFormatter.format(selectedDateTime),
                        style: Theme.of(context).textTheme.titleMedium?.copyWith(
              ...
              actions: [
                ...
                TextButton(
                  child: const Text('Confirmer'),
                  onPressed: () => Navigator.of(dialogContext).pop(selectedDateTime),
                ),
              ],
            );
          },
  ...
  // --- FIN DE LA NOUVELLE MODALE ---

  // GESTION DES STATUTS AVEC PLANIFICATION (Modifié pour appeler la nouvelle modale)
  Future<void> _handleScheduleUpdate(BuildContext ctx, Order order, String newStatus, DateTime defaultDateTime) async {
    final currentContext = context;
    if (!mounted) return;

    String dialogTitle = newStatus == 'A relancer' ? 'Planifier Relance #${order.id}' : 'Reporter Commande #${order.id}';
    bool isRelaunch = newStatus == 'A relancer';

    // *** APPEL DE LA NOUVELLE MODALE INTUITIVE ***
    final DateTime? scheduledAt = await _showScheduleModal(currentContext, dialogTitle, defaultDateTime, isRelaunch);
    ...
    if (scheduledAt != null) {
      ...
      final successMessage = newStatus == 'A relancer'
          ? 'Commande #${order.id} marquée À relancer le ${DateFormat('dd/MM à HH:mm').format(scheduledAt)}.'
          : 'Commande #${order.id} Reportée au ${DateFormat('dd/MM à HH:mm').format(scheduledAt)}.';

      _performApiAction(
        // Utilise la nouvelle signature de updateOrderStatus
  ...
  Future<void> _handleStatusUpdate(BuildContext ctx, Order order) async {
    ...
    final result = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (BuildContext sheetContext) {
        ...
        final followUpColor = AppTheme.followUpColor; // Violet

        // --- NOUVELLE LOGIQUE DE FILTRAGE DES ACTIONS ---
        
        // Statuts de suivi (toujours dispos si 'Statuer' est cliqué)
        final List<Widget> followUpActions = [
          ListTile(leading: Icon(Icons.phone_missed_outlined, color: followUpColor), title: const Text('Ne décroche pas'), onTap: () => Navigator.pop(sheetContext, {'status': 'Ne decroche pas'})),
          ListTile(leading: Icon(Icons.person_off_outlined, color: followUpColor), title: const Text('Injoignable'), onTap: () => Navigator.pop(sheetContext, {'status': 'Injoignable'})),
          ListTile(leading: Icon(Icons.autorenew_outlined, color: followUpColor), title: const Text('À relancer'), onTap: () => Navigator.pop(sheetContext, {'status': 'A relancer'})),
          ListTile(leading: Icon(Icons.calendar_month_outlined, color: followUpColor), title: const Text('Reportée'), onTap: () => Navigator.pop(sheetContext, {'status': 'Reportée'})),
        ];

        // Statuts de clôture (dispos seulement si en route ou en suivi)
        // Le livreur doit avoir démarré la course (en_route) ou être en suivi pour clôturer.
        final List<String> closingAllowedStatuses = ['en_route', 'reported', 'Ne decroche pas', 'Injoignable', 'A relancer', 'Reportée'];
        final bool canClose = closingAllowedStatuses.contains(order.status);

        final List<Widget> closingActions = [
          ...
          ListTile(leading: const Icon(Icons.error_outline, color: Colors.orange), title: const Text('Livraison ratée'), onTap: () => Navigator.pop(sheetContext, {'status': 'failed_delivery'})),
        ...
        return SafeArea(
          child: Wrap(
            children: [
              // N'affiche les actions de clôture que si c'est autorisé
              if (canClose) ...closingActions,
              // Ajoute un séparateur si les deux listes sont affichées
              if (canClose && followUpActions.isNotEmpty) const Divider(),
              // Affiche toujours les actions de suivi
              ...followUpActions,
            ],
          ),
        );
    ...
    if (result != null && result['status'] != null) {
      ...
      } else if (status == 'A relancer') {
         // Ouvre la modale de planification, date par défaut: Aujourd'hui (et priorise l'heure)
         _handleScheduleUpdate(currentContext, order, status, DateTime.now());
      } else if (status == 'Reportée') {
         // Ouvre la modale de planification, date par défaut: Demain (et priorise la date)
         _handleScheduleUpdate(currentContext, order, status, DateTime.now().add(const Duration(days: 1)));
      }
      else if (['reported', 'Ne decroche pas', 'Injoignable'].contains(status)) {
         // Statut simple sans planification
         _performApiAction(
          () => orderService.updateOrderStatus(orderId: order.id, status: status, paymentStatus: 'pending'), 
          'Commande #${order.id} marquée comme $status.',
        );
      }
    }
  }
...
class OrderCard extends StatelessWidget {
  ...
  // (Helpers _getStatusColor, _getStatusIcon, paymentTranslations, etc. - AJOUT DES NOUVEAUX STATUTS)
  Color _getStatusColor(String status) {
     switch (status) {
      ...
      case 'reported':
      case 'Ne decroche pas': // Nouveau
      case 'Injoignable':     // Nouveau
      case 'A relancer':      // Nouveau
      case 'Reportée':        // Nouveau
        return AppTheme.followUpColor; // Violet
  ...
  IconData _getStatusIcon(String status) {
      ...
      case 'Ne decroche pas': return Icons.phone_missed_outlined;    // Nouveau
      case 'Injoignable': return Icons.person_off_outlined;           // Nouveau
      case 'A relancer': return Icons.autorenew_outlined;             // Nouveau
      case 'Reportée': return Icons.calendar_month_outlined;          // Nouveau
  ...
  Widget build(BuildContext context) {
    final orderService = Provider.of<OrderService?>(context, listen: false);
    // Utilisation de statusTranslations (qui contient maintenant les nouveaux statuts)
    ...
    // *** CORRECTION LOGIQUE (DEMANDE UTILISATEUR) ***
    // 'Statuer' est dispo dès que la course est assignée (in_progress) ou prête (ready_for_pickup),
    // ou en cours (en_route), ou déjà en suivi (reported, A relancer, etc.)
    final List<String> updatableStatuses = [
        'in_progress', 
        'ready_for_pickup', 
        'en_route', 
        'reported', 
    ...
    final bool canUpdateStatus = updatableStatuses.contains(order.status);

    // Statuts qui autorisent la déclaration de retour (y compris les nouveaux)
    final List<String> returnableStatuses = ['en_route', 'failed_delivery', 'cancelled', 'reported', 'Ne decroche pas', 'Injoignable', 'A relancer', 'Reportée'];
    final bool canDeclareReturn = returnableStatuses.contains(order.status);
    
    // Affichage conditionnel de la date de suivi
    final bool isFollowUpStatus = ['reported', 'A relancer', 'Reportée'].contains(order.status);
         ...
         child: Padding(
           ...
           child: Column(
             ...
             children: [
               ...
               // NOUVEAU: Affichage de la date de suivi
               if (isFollowUpStatus && order.followUpAt != null)
                   _buildDetailRow(
                      Icons.schedule, 
                      // MODIFIÉ: Logique d'affichage du label
                      order.status == 'A relancer' ? 'Relance Prévue' : (order.status == 'Reportée' ? 'Reporté au' : 'Suivi le'), 
                      DateFormat('dd MMM yyyy à HH:mm', 'fr_FR').format(order.followUpAt!), 
                      iconColor: AppTheme.followUpColor
                   ),
  ...
  // Widget helper pour afficher une ligne de détail (mis à jour pour la couleur)
  Widget _buildDetailRow(IconData icon, String label, String value, {int maxLines = 1, Color? iconColor}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 6.0),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(icon, size: 18, color: iconColor ?? Colors.grey.shade600), // Utilise iconColor
          ...
          Text(
           ...
           Expanded(
            child: Text(
              value,
              style: TextStyle(fontWeight: FontWeight.w500, fontSize: 14, color: iconColor ?? AppTheme.text),
              overflow: TextOverflow.ellipsis,
              maxLines: maxLines,
            ),
          ),
