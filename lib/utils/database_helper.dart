...
class DatabaseHelper {
  ...
  static const String _ordersTableName = 'orders'; // NOUVEAU
  static const int _dbVersion = 3; // Incrémenté à 3 pour la table orders
  ...
  Future<Database> _initDB() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        // Crée la table messages (v2)
        await _createMessagesTable(db);
        // Crée la table orders (v3)
        await _createOrdersTable(db);
      },
      onUpgrade: (db, oldVersion, newVersion) async {
        // Logique de migration
        if (oldVersion < 2) {
          debugPrint("DB Upgrade: Création table $_messagesTableName (v2)");
          await _createMessagesTable(db);
        }
        if (oldVersion < 3) {
          debugPrint("DB Upgrade: Création table $_ordersTableName (v3)");
          await _createOrdersTable(db);
        }
      },
    );
  ...
  // NOUVELLE FONCTION: Créer la table des commandes (pour le mode hors ligne)
  Future<void> _createOrdersTable(Database db) async {
     await db.execute('''
      CREATE TABLE IF NOT EXISTS $_ordersTableName (
          ...
          items_list TEXT,
          is_picked_up INTEGER NOT NULL,
          follow_up_at TEXT 
      )
    ''');
    // Index
    await db.execute('CREATE INDEX IF NOT EXISTS idx_order_status ON $_ordersTableName (status)');
    await db.execute('CREATE INDEX IF NOT EXISTS idx_order_follow_up ON $_ordersTableName (follow_up_at)');
    debugPrint("DB Helper: Table '$_ordersTableName' créée/vérifiée.");
  }
  ...
  // --- Les fonctions CRUD pour la table orders (à implémenter dans OrderService) ---
  /*
  Future<void> insertOrder(Order order) async { ... }
  Future<List<Order>> getOrdersLocally() async { ... }
  */
}